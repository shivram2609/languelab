...
*/	
	function signup() {
		...
		if (isset($this->data) && !empty($this->data)) {
			...
			$errflag = 0;
			$this->Userdetail->validate['first_name']['message'] = 'Please enter name.';
			if ($this->User->validates()) {
				$errflag += 1;
			}
			...
			if ($errflag == 2) {
				...
				if ($this->User->save($user,array('validate' => false))) {
					...
					//die;
					/* code to send email confirmation for signup */
					$this->Session->setFlash("Your registration with LMS has been successful and a confirmation email has been sent to your registered email address.", 'default', array("class"=>"success_message"));
					$this->redirect(array("action"=>"login"));
				}
			}
		}
